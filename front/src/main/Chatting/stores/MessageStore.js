...
export default class MessageStore {
  ...
  async connectAndSubscribe(post_no) {
    try {
  ...
  subscribeMessageBroker(post_no) {
    this.client.connect({}, () => {
  ...
  disconnect = () => {
    ...
    if (this.connected) {
      ...
      console.warn("WebSocket 연결이 아직 수립되지 않았습니다. 연결 해제를 건너뜁니다.");
    }
  }
  ...
  sendMessage({ type }) {
    ...
    if (this.connected) {
      ...
      console.error("연결이 아직 수립되지 않았습니다. 메시지를 보낼 수 없습니다.");
    }
